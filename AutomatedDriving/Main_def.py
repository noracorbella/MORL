...
if __name__ == "__main__":
    ...
    # STOCHASTIC ########################################
        ...
        from CHVI_stochastic import convexhull_VI, get_initial_state_hull
        ...
        if algorithm_used == STOC_VALUE_ITERATION:
            ...
            chvi_policy_name = os.path.join(PDIR, f"CHVI_stochastic_{p[0]}-{p[1]}-{p[2]}-policy.npy")
        ...
        if algorithm_used == STOC_CONVEX_HULL_VI:
            if Calculate_hulls:
                ...
                np.save(hulls_name, q_hulls, allow_pickle=True)
                print(f"Saved Q-hulls to {hulls_name}\n")
                policy = extract_lex_policy_from_qhulls(q_hulls, p, env, env.n_actions)
                np.save(chvi_policy_name, policy)
                print(f"Saved policy to {chvi_policy_name}\n")
            else:
                print(f"Loading Q-hulls from {hulls_name}...\n")
        ...
        if Evaluate_policy:
            ...
            if algorithm_used == STOC_VALUE_ITERATION:
                ...
                chvi_policy_file = os.path.join(PDIR, f"CHVI_stochastic_{p[0]}-{p[1]}-{p[2]}-policy.npy")
                try:
                    ...
                    try:
                        ...
                        policy = extract_lex_policy_from_qhulls(Q_hulls, p, env_temp, env_temp.n_actions)
            ...
            if algorithm_used == STOC_VALUE_ITERATION and weights is not None:
    ...
    if Load_V_at_initial_state:
        ...
        load_priority = p if domain == STOCHASTIC else (priority if domain == LEXICOGRAPHIC else None)
        ...
        env_load = Environment(weights=None)
        env_load.reset()
