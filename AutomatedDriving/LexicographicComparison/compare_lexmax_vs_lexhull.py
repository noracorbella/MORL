...
import time
import gc
from LG_utils import generate_all_priority_orders, lex_hull, lex_max
from LG_VI_stoc_lexmax import LG_VI_lexmax
from LG_VI_stoc_lexhull_independent_Qs import LG_VI_lexhull
...
def main(quick_test=False):
    ...
    for i, priority in enumerate(priorities_to_test):
        ...
        # Only get Q from first run
        if i == 0:
            policy, Q = LG_VI_lexmax(env, theta=theta, discount_factor=discount_factor, 
                                    priority=priority, return_Q=True)
        else:
            policy, Q = LG_VI_lexmax(env, theta=theta, discount_factor=discount_factor, 
                                    priority=priority, return_Q=False)
        ...
        if lexmax_Q_reference is None and Q is not None:
            ...
            # Verify Q consistency if available
            if Q is not None:
                q_consistency = compare_q_values(Q, lexmax_Q_reference, f"Priority {priority_tuple}")
                if q_consistency['match']:
                    print(f"Q-table matches reference (max diff: {q_consistency['max_diff']:.2e})")
                else:
                    print(f"WARNING: Q-table differs from reference!")
