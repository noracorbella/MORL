...
def LG_VI_lexhull(env, theta=1.0, discount_factor=0.7, priority=[0,1,2]):
    ...
    # Initialize value function and Q-table
    ...
    # Reference priority for convergence check (Option B)
            ...
            for c in env.states_agent_left:
                for p1 in env.states_agent_right:
                    for p2 in env.states_agent_right:
                        ...
                        # Use lex_hull to get optimal actions for all priorities
                        lex_optimal_actions = lex_hull(q_vectors, n_objectives=n_objectives)

                        # For convergence, use reference priority (Option B)
    ...
    # Extract policies for all priority orders
    print("\nExtracting policies for all priority orders...")
