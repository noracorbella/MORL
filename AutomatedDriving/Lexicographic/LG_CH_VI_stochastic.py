...
def lex_max_hull(hulls_per_action, priority='car', tol=1e-9):
    """
    ...
    for action in range(n_actions):
        ...
        for vector in hull:
            ...
            action_for_vector.append(action)
    ...
    for i in range(1, len(all_vectors)):
        for obj_idx in objective_order:
            diff = all_vectors[i][obj_idx] - all_vectors[best_idx][obj_idx]
            if diff > tol: # vector[i] is better
                best_idx = i
                break
            elif diff < -tol: # best_idx is still better
                break
            # If equal, continue to next objective
    
    best_vector = all_vectors[best_idx]
    best_action = action_for_vector[best_idx]
    
    # print(f"Best vector: {best_vector}")
    # print(f"From action: {best_action}")
    ...
    #         max_values[action] = max_val_in_hull

    #     global_max = max(max_values.values()) # global max for this obj

    #     new_best_actions = [
    #         action for action in best_actions
    #         if abs(max_values[action] - global_max) < tol
    #     ]
