...
----------
  ...
  3. LGVI-H : Lexicographic Hull Value Iteration + extract 2 policies in one pass.
"""
...
POLICIES_DIR     = os.path.join(_HERE, "dst_benchmark_policies")
RESULTS_DIR      = os.path.join(_HERE, "dst_benchmark_results")
...
# files
CHVI_MNS_FILE         = os.path.join(POLICIES_DIR, "DST_CHVI_MNS.pkl")
CHVI_QHULLS_FILE      = os.path.join(POLICIES_DIR, "DST_CHVI_qhulls.pkl")
LGVI_LEXMAX_MNS_FILE  = os.path.join(POLICIES_DIR, "DST_LGVI_lexmax_MNS.pkl")
LGVI_LEXHULL_MNS_FILE = os.path.join(POLICIES_DIR, "DST_LGVI_lexhull_MNS.pkl")
LGVI_LEXHULL_V_FILE   = os.path.join(POLICIES_DIR, "DST_LGVI_lexhull_V_hulls.pkl")
LGVI_LEXHULL_Q_FILE   = os.path.join(POLICIES_DIR, "DST_LGVI_lexhull_Q_hulls.pkl")
...
def run_benchmark1(theta, discount_factor):
    ...
    for priority_label, priority in LEX_ORDERS:
        ...
        np.save(os.path.join(POLICIES_DIR, f"DST_CHVI_lexmax_{priority[0]}-{priority[1]}_policy.npy"), policy,)
...
def run_benchmark2(theta, discount_factor):
    ...
    for label, priority in LEX_ORDERS:
        _delete_if_exists(os.path.join(POLICIES_DIR, f"DST_LGVI_lexmax_{priority[0]}-{priority[1]}_policy.npy"))
    ...
    for label, priority in LEX_ORDERS:
        policy_file = os.path.join(POLICIES_DIR, f"DST_LGVI_lexmax_{priority[0]}-{priority[1]}_policy.npy")
...
def run_benchmark3(theta, discount_factor):
    ...
    for label, priority in LEX_ORDERS:
        _delete_if_exists(os.path.join(POLICIES_DIR, f"DST_LGVI_lexhull_{priority[0]}-{priority[1]}_policy.npy"))
