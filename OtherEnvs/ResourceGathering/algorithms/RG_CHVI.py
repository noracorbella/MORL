...
def convexhull_VI(env, theta=1.0, discount_factor=0.7,
                  MNS_filename='rg_policies/RG_CHVI_MNS.pkl',
                  q_hulls_file=None):
    """
    Convex Hull Value Iteration for Resource Gathering (stochastic MOMDP).
    """
    ...
    while True:
        ...
        with tqdm(total=total_states, desc=f"Iteration {iteration}") as pbar:
            ...
            for state in env.valid_states:
                ...
                # if v_old.shape == new_V.shape:
                #     v_old_sorted = v_old[np.lexsort(v_old.T)]
                #     new_V_sorted = new_V[np.lexsort(new_V.T)]
                #     max_diff = np.max(np.abs(new_V_sorted - v_old_sorted))
                # else:
                #     max_diff = float('inf')
...
def extract_policy_for_weights(Q_hulls, weights, env, n_actions):
    ...
    print(f"[w_enemy={weights[0]:.2f}, w_gold={weights[1]:.2f}, w_gem={weights[2]:.2f}]")
