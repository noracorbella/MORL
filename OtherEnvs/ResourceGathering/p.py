...
if __name__ == "__main__":
    ...
    if Load_V_at_initial_state:
        print(f"\n{'='*60}")
        print("VALUE AT INITIAL STATE (loaded from file)")
        print(f"{'='*60}")

        # ---- Pick which file to load ---------------------------------
        # Options:
        #   VI       :  os.path.join(PDIR, f"RG_V_table_{w0}-{w1}-{w2}_vec.pkl")
        #   CHVI     :  os.path.join(PDIR, "RG_CHVI_vhulls.pkl")
        #   LexMax   :  os.path.join(PDIR, f"RG_LGVI_lexmax_{p0}-{p1}-{p2}_V_table.pkl")
        #   LexHull  :  os.path.join(PDIR, "RG_LGVI_lexhull_V_hulls.pkl")

        load_file     = os.path.join(PDIR, "RG_CHVI_vhulls.pkl")
        load_weights  = [1, 10, 100]   # used by CHVI (vertex selection)
        load_priority = [0, 1, 2]      # used by LexHull (vertex selection)
