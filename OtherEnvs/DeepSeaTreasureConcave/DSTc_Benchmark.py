...
BENCHMARKS
----------
  1. CHVI   : Convex Hull Value Iteration + extract 2 lexicographic policies via lex_max.
  2. LGVI   : 2x Lexicographic Value Iteration (one run per priority order).
  3. LGVI lexhull : Lexicographic Hull Value Iteration + extract 2 policies in one pass.
"""
...
POLICIES_DIR     = os.path.join(_HERE, "dstc_benchmark_policies")
RESULTS_DIR      = os.path.join(_HERE, "dstc_benchmark_results")
...
BENCHMARK_TO_RUN = BENCHMARK3_LGVI_LEXHULL # BENCHMARK1_CHVI | BENCHMARK2_LGVI_LEXMAX | BENCHMARK3_LGVI_LEXHULL
...
# files
CHVI_MNS_FILE         = os.path.join(POLICIES_DIR, "DSTc_CHVI_MNS.pkl")
CHVI_QHULLS_FILE      = os.path.join(POLICIES_DIR, "DSTc_CHVI_qhulls.pkl")
LGVI_LEXMAX_MNS_FILE  = os.path.join(POLICIES_DIR, "DSTc_LGVI_lexmax_MNS.pkl")
LGVI_LEXHULL_MNS_FILE = os.path.join(POLICIES_DIR, "DSTc_LGVI_lexhull_MNS.pkl")
LGVI_LEXHULL_V_FILE   = os.path.join(POLICIES_DIR, "DSTc_LGVI_lexhull_V_hulls.pkl")
LGVI_LEXHULL_Q_FILE   = os.path.join(POLICIES_DIR, "DSTc_LGVI_lexhull_Q_hulls.pkl")
...
# ---------------------------------------------------------------------------
# BENCHMARK 1: Convex Hull Value Iteration + Extract 2 Lexicographic Policies
...
def run_benchmark1(theta, discount_factor):
    ...
    for priority_label, priority in LEX_ORDERS:
        ...
        np.save(os.path.join(POLICIES_DIR, f"DSTc_CHVI_lexmax_{priority[0]}-{priority[1]}_policy.npy"), policy,)
...
# Measures the time to run LG_VI_lexmax separately for each of the 2 lexicographic priority orders and extract the optimal policy for each.
#   1. Delete MNS cache before first LG order
#   2. For order 2 reuse the cached MNS
# ---------------------------------------------------------------------------
...
def run_benchmark2(theta, discount_factor):
    ...
    for label, priority in LEX_ORDERS:
        _delete_if_exists(os.path.join(POLICIES_DIR, f"DSTc_LGVI_lexmax_{priority[0]}-{priority[1]}_policy.npy"))
    ...
    for label, priority in LEX_ORDERS:
        policy_file = os.path.join(POLICIES_DIR, f"DSTc_LGVI_lexmax_{priority[0]}-{priority[1]}_policy.npy")
...
def run_benchmark3(theta, discount_factor):
    ...
    for label, priority in LEX_ORDERS:
        _delete_if_exists(os.path.join(POLICIES_DIR, f"DSTc_LGVI_lexhull_{priority[0]}-{priority[1]}_policy.npy"))
    ...
    for priority_tuple, policy in policies.items():
        policy_file = os.path.join(POLICIES_DIR, f"DSTc_LGVI_lexhull_{priority_tuple[0]}-{priority_tuple[1]}_policy.npy")
        np.save(policy_file, policy)
