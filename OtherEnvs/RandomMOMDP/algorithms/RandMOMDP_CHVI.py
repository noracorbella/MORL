...
def convexhull_VI(env, theta=1.0, discount_factor=0.7,
                  MNS_filename='randmomdp_policies/RandMOMDP_CHVI_MNS.pkl',
                  q_hulls_file=None):
    """
    Convex Hull Value Iteration for the random MOMDP (stochastic, integer states).
    """
    ...
    while True:
        ...
        # Jacobi-style: freeze all hulls before the sweep so no state
        # reads a hull that was already updated in this iteration.
        ...
        with tqdm(total=total_states, desc=f"Iteration {iteration}") as pbar:
            ...
            for state in env.valid_states:
                ...
                # V(s) = CH of union of all Q-hulls across available actions
                all_q_vectors = []
                for action in valid_actions:
                ...
                if len(all_q_vectors) == 0:
                    # Terminal state with no available actions.
                    new_V = np.array([np.zeros(n_rewards)])
                else:
