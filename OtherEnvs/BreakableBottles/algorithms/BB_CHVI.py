...
def convexhull_VI(env, theta=1.0, discount_factor=0.95,
    ...
    log_path = os.path.join(os.path.dirname(MNS_filename) if os.path.dirname(MNS_filename) else '.', 'debug.log')
    log_file = open(log_path, 'w')
    log_file.write("iteration\tdelta\tavg_hull_size\n")
    ...
    while True:
        ...
        log_file.write(f"{iteration}\t{delta}\t{avg_hull_size}\n")
        log_file.flush()
