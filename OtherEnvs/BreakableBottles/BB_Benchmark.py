...
BB_Benchmark.py
---------------
Unified benchmark runner for the BreakableBottles algorithms.
...
POLICIES_DIR = os.path.join(_HERE, "bb_benchmark_policies")
RESULTS_DIR  = os.path.join(_HERE, "bb_benchmark_results")
...
# Files
CHVI_MNS_FILE         = os.path.join(POLICIES_DIR, "BB_CHVI_MNS.pkl")
CHVI_QHULLS_FILE      = os.path.join(POLICIES_DIR, "BB_CHVI_qhulls.pkl")
LGVI_LEXMAX_MNS_FILE  = os.path.join(POLICIES_DIR, "BB_LGVI_lexmax_MNS.pkl")
LGVI_LEXHULL_MNS_FILE = os.path.join(POLICIES_DIR, "BB_LGVI_lexhull_MNS.pkl")
LGVI_LEXHULL_V_FILE   = os.path.join(POLICIES_DIR, "BB_LGVI_lexhull_V_hulls.pkl")
LGVI_LEXHULL_Q_FILE   = os.path.join(POLICIES_DIR, "BB_LGVI_lexhull_Q_hulls.pkl")

# 6 priority orders for BB (3 objectives -> 3! = 6)
LEX_ORDERS = [
    ("[0, 1, 2]", [0, 1, 2]),
    ("[0, 2, 1]", [0, 2, 1]),
    ("[1, 0, 2]", [1, 0, 2]),
...
# ---------------------------------------------------------------------------
# BENCHMARK 1: CHVI + Extract 6 Lexicographic Policies
# ---------------------------------------------------------------------------
...
def run_benchmark1(theta, discount_factor):
    ...
    for priority_label, priority in LEX_ORDERS:
        ...
        np.save(
            os.path.join(POLICIES_DIR, f"BB_CHVI_lexmax_{priority[0]}-{priority[1]}-{priority[2]}_policy.npy"),
            policy,
        )
...
def run_benchmark2(theta, discount_factor):
    ...
    for label, priority in LEX_ORDERS:
        _delete_if_exists(os.path.join(POLICIES_DIR, f"BB_LGVI_lexmax_{priority[0]}-{priority[1]}-{priority[2]}_policy.npy"))

    run_times     = {}
    t_total_start = time.time()

    for label, priority in LEX_ORDERS:
        policy_file = os.path.join(POLICIES_DIR, f"BB_LGVI_lexmax_{priority[0]}-{priority[1]}-{priority[2]}_policy.npy")
    ...
    lines.append(f"Average per run: {(total_time / 6):.3f} s")
...
def run_benchmark3(theta, discount_factor):
    ...
    for label, priority in LEX_ORDERS:
        _delete_if_exists(os.path.join(POLICIES_DIR, f"BB_LGVI_lexhull_{priority[0]}-{priority[1]}-{priority[2]}_policy.npy"))

    print("\nRunning Lexicographic Hull Value Iteration...")
    env = BBEnvironment(weights=None, **ENV_KWARGS)
    ...
    del Q_hulls
    timing_LHVI = timing['model_build'] + timing['remaining_vi']

    print(f"\nElapsed time for LG_VI_lexhull: {total_time:.3f} s")

    print("\nSaving extracted policies...")
    for priority_tuple, policy in policies.items():
        p_str       = f"{priority_tuple[0]}-{priority_tuple[1]}-{priority_tuple[2]}"
        policy_file = os.path.join(POLICIES_DIR, f"BB_LGVI_lexhull_{p_str}_policy.npy")
        np.save(policy_file, policy)
    ...
    lines.append(f"LHVI:  {timing_LHVI:.3f}")
