...
print("\n" + "=" * 60)
print("QUERY: best vertex for a given weight vector")
print("=" * 60)

query_weight = np.array([0.333, 0.333, 0.334])
query_weight = query_weight / query_weight.sum()  # normalise just in case

best_idx = np.argmax(vertices_chvi @ query_weight)
print(f"\nWeight vector: [{query_weight[0]:.4f}, {query_weight[1]:.4f}, {query_weight[2]:.4f}]")
print(f"Best vertex: {vertices_chvi[best_idx]}")