...
if __name__ == "__main__":
    ...
    if Load_V_at_initial_state:
        ...
        if algorithm_used == VALUE_ITERATION:
            load_file = os.path.join(PDIR, f"BB_V_table_{weights[0]}-{weights[1]}-{weights[2]}_vec.pkl")
        elif algorithm_used == CONVEX_HULL_VI:
            load_file = os.path.join(PDIR, "BB_CHVI_vhulls.pkl")
        elif algorithm_used == LGVI_LEXMAX:
            load_file = os.path.join(PDIR,f"BB_LGVI_lexmax_{priority[0]}-{priority[1]}-{priority[2]}_V_table.pkl")
        elif algorithm_used == LGVI_LEXHULL:
            load_file = os.path.join(PDIR, "BB_LGVI_lexhull_V_hulls.pkl")
        ...
        if data is not None:
            ...
            if "BB_V_table" in fname and "_vec" in fname:
            ...
            elif "BB_LGVI_lexmax" in fname and "V_table" in fname:
                V_s0 = np.asarray(data[s0])
                print(f"File         : {fname}")
            ...
            elif "BB_LGVI_lexhull_V_hulls" in fname:
                value_hull = np.asarray(data[s0])
                ordered    = value_hull[:, priority]
